...
std::string fcn_form_field_value(const std::string& value, std::size_t width)
{
...
void f3_svd_test_mask(const GnFormContext& context)
{
    ...
    fcn_form_write(first_field_row, 5, "  Numero di righe n", field_column - 6);
    fcn_form_write(first_field_row + 2, 5, "  Numero di colonne d", field_column - 6);
...
void f3_svd_test_show(const GnFormContext& context)
{
    ...
    for (std::size_t i = 0; i < context.field_count; ++i) {
        ...
        fcn_form_write(
            ...
            fcn_form_field_value(context.values[i], context.fields[i].length),
 ...
 */
GnFormChangeResult fcn_form_change_linear(
...
GnFormChangeResult f3_svd_test_change(
    ...
    void*)
{
    return fcn_form_change_linear(command, context);
}
...
void f3_svd_pca_mask(const GnFormContext& context)
{
    ...
    fcn_form_write(first_field_row,     5, "  Campioni base n",        field_column - 6);
    fcn_form_write(first_field_row + 2, 5, "  Dimensione base d",      field_column - 6);
    fcn_form_write(first_field_row + 4, 5, "  Moltiplicatore n",       field_column - 6);
    fcn_form_write(first_field_row + 6, 5, "  Moltiplicatore d",       field_column - 6);
...
void fcn_form_clear_pending_stdin()
{
    if (std::cin.fail()) {
        std::cin.clear();
